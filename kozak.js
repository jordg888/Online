    ...
    function KozakTiv() {
        ...
        Lampa.Component.add('kozak_mod', function(object) {
            ...
            this.create = function() {
                ...
                // Налаштування вибору балансерів зверху
                ...
                filter.render().find('.filter__sort').remove(); // Видаляємо непотрібне сортування
                
                scroll.append(filter.render());
                scroll.append(files.render());

                this.search('ashdi'); // Починаємо з Ashdi
            ...
            this.search = function(source_type) {
                var _this = this;
                // ФІКС: замість files.clear() просто очищуємо контейнер вручну
                files.render().empty(); 
                Lampa.Select.show({title: 'Пошук на ' + source_type + '...'});
                ...
                network.silent(final_url, function(json) {
                    ...
                    if (items && items.length > 0) {
                        ...
                        Lampa.Noty.show('На цьому балансері нічого не знайдено');
                    }
                }, function() {
                    Lampa.Select.close();
                    Lampa.Noty.show('Помилка проксі-сервера');
                });
            };
